...
## 2026-09-25 — Visual polish: TOS tree, Reviewers tables, dialog shadow, and the dashboard's wider layout rolled out to every section

Gabriel asked for a further "make it beautiful" pass, then specifically said he liked the dashboard's new layout and wanted it applied everywhere it made sense. Three scoped pieces of work:

**Question Bank / TOS (`question-bank-browser.tsx`, `question-bank/page.tsx`):** the Official Subject / Subject / Topic levels were each their own nested bordered-and-radiused box (a box inside a box inside a box), which read as heavy for a 3-4 level tree. Flattened all three nested levels to hairline `border-b` separators with a hover-highlight instead — the top-level Area card keeps its full border/accent treatment (it's the primary grouping), everything below it now reads as a clean indented tree. Area title bumped to `text-base` for clearer top-level hierarchy. Container widened `max-w-3xl` → `max-w-4xl`.

**Reviewers tables (`reviewers-browser.tsx`):** added per-column numeric alignment — a column right-aligns (header included) when most of its data rows are bare numbers/units ("998.2 kg/m³"), decided per column (not per cell) so a header never ends up misaligned against its own values. The heuristic explicitly excludes short-but-word-containing cells ("Mild Steel A36") so labels don't false-positive. Row height bumped `py-1.5` → `py-2` for more comfortable scanning.

**`ui/dialog.tsx`:** found and fixed a real inconsistency during a visual-consistency audit — `DialogContent` had a `ring` but no `shadow`, while `DropdownMenuContent`/`SelectContent` both already had `shadow-md`. Added `shadow-xl` (dialogs are more prominent/page-blocking than a dropdown, so slightly stronger than the other two).

**Dashboard-style width rollout** — the same principle from the dashboard fix (widen the container, let internal grids use the space, never blindly stretch content) applied across every other list/browse page under `(app)/`: Practice (`max-w-3xl`→`5xl`), Progress (`3xl`→`5xl`, plus its 6-stat Preparation Profile grid gained `lg:grid-cols-6` for a single row), Mistakes/Notes (`2xl`→`4xl`), Recalled (`3xl`→`5xl`), Settings/Profile (`3xl`→`4xl`), Study Plan (`2xl`→`3xl`, kept more modest since it's mostly a settings-style form), PAES hub (card grid gained `lg:grid-cols-4` for a single row of 4), PAES Mastery (`3xl`→`5xl`). Flashcards went further: both its area-picker (3 cards) and per-area action list (4 cards) were flat vertical stacks — converted to `sm:grid-cols-3` / `sm:grid-cols-2` respectively so the extra width does real work instead of leaving each card looking like a thin bar with empty space beside it. Single-focus pages were deliberately left narrow — Quiz Builder, Quick Practice, the PAES Quiz picker, and the Flashcards study view are forms/single-card reading experiences where extra width would just create awkward whitespace, matching the "don't blindly stretch everything" principle from the original dashboard-composition request.

**Bonus bug fix, found while touching these pages:** the Flashcards area-picker cards and the PAES hub cards both had the same dead `hover:border-{color}` bug already fixed on the dashboard earlier this session (the shared `Card` component has no `border` utility, only `ring-1`, so a border-color-only hover override was invisible). Both now use the working `hover:ring-primary/40` + lift + shadow pattern.

**Testing performed:** visually verified Practice, Flashcards (both picker and area-action views, at 1600px), Progress, and the PAES hub at desktop width, and Progress at 375px mobile (no horizontal overflow). `npm run lint` and `npm run build` both clean.
